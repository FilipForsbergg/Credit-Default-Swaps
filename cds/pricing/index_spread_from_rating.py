...
def plot_spreads(df):
    ...
    plt.tight_layout()
    plt.savefig("plots/cds_spread_plot.png")
    plt.show()
...
def main():
    ...
    pipeline = CDS(params)
    df['cds_flat_spread'] = df['RATING'].apply(lambda x: pipeline.flat_spread(x) * 10000)
    
    adv_result = pipeline.index_from_component_spreads(df)
